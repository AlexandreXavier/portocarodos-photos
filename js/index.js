(async function () {
  ...
  try {
    ...
    updatePrompt('<p>Algo está errado.</p><a href="https://github.com/AlexandreXavier/dat-photos-app">Report an issue</a>')
  }
  ...
  async function appendAlbum (album) {
    ...
    albumHTML += `
      <div class="dropdown" data-album="${album.url}">
        <div class="delete-album-btn" data-album="${album.url}">Apagar album</div>
      </div>
    `

    if (!images.length) {
      el.classList.add('empty')
      albumHTML += '<div class="placeholder">Sem Fotos</div>'
    } else {
  ...
  function renderUAPrompt () {
    updatePrompt('<p>Desculpe >.< Esta aplicação só trabalha com o Beaker Browser.</p><a class="btn primary" href="https://beakerbrowser.com/docs/install/">Instalar Beaker</a>')
  }
